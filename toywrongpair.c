...
int main() {
	...
	count = 0;
	ncount=0;
	for(loop = 1; loop < (1<<12); loop++){
	    for(int i=0; i < 4;i++){
	        c1[i] = rand() & 0xff;
		c2[i] = c1[i];
		for(int j = 0; j < 3; j++)
		    c2[i] = c2[i] ^ (((loop >> 3*i+j)&1) << (7-col3[j]) );
	...
	printf("\n\t\t Average number of keys survived per wrong pair after Stage-1 = %d ",count/(loop-1));
	
	printf("\n\t\t Average number of keys survived for all wrong pair after Stage-1 = %d ",count);
	
	printf("\n\t\t Average number of keys survived per wrong pair after Stage-1 + Stage-2 = %d", ncount/(loop-1));
	
	printf("\n\t\t Average number of keys survived for all wrong pair after Stage-1 + Stage-2 = %d\n", ncount);
